...
class TransferLearningEngine: # 전이 학습 엔진 클래스 정의
    ...
    def save_combined_model_as_pt(self, file_name): # 기본 모델과 LoRA 가중치가 결합된 전체 모델의 state_dict를 .pt 파일로 저장하는 메서드
        try:
            ...
            # LoRA 가중치를 v4 모델에 병합 (원본 모델을 복사해서 사용)
            # 원본 PEFT 모델의 상태를 보존하기 위해 복사본에서 merge_and_unload 수행
            import copy
            estimator_copy = copy.deepcopy(self._estimator)
            merged_v4_model = estimator_copy.merge_and_unload() # LoRA 가중치를 v4 모델에 병합
            # v4 구조 그대로 저장 (engine_v4.py와 호환)
            torch.save(merged_v4_model, full_path)
            print(f"v4 model saved to {full_path}")
...
if __name__ == "__main__": # InF 환경 특화 전이학습 실행
    ...
    # InF 환경 전용 설정 파일 사용
    conf_file = 'config_transfer_v4_InF.yaml' # InF 환경 특화 설정 파일 사용
    
    print("=" * 60)
    print("InF (Indoor Factory) LoRA Transfer Learning Start")
    print("=" * 60)
    print(f"Config file: {conf_file}")
    print(f"Target environment: Indoor Factory (Los/Nlos)")
    print(f"Transfer method: LoRA (Low-Rank Adaptation)")
    print("=" * 60)
    
    engine = TransferLearningEngine(conf_file) # TransferLearningEngine 객체 생성
    engine.load_model() # v4 베이스 모델 로드 및 LoRA 적용
    
    print("InF transfer learning starting...")
    engine.train() # InF 데이터로 LoRA 전이학습 수행
    
    print("=" * 60)
    print("InF LoRA transfer learning completed!")
    ...
    try:
        final_model_name = engine._conf['training'].get('saved_model_name', 'Large_estimator_v4_to_InF_lora')
